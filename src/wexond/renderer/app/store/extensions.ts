...
export class ExtensionsStore {
  ...
  public queryBrowserAction(query: Record<string, unknown>): BrowserAction[] {
    const readProperty = (obj: Record<string, unknown>, prop: string) => obj[prop]

    return this.browserActions.filter(item => {
      for (const key in query) {
        const itemProp = readProperty(item as Record<string, unknown>, key)
  ...
  public load(): void {
    const extensions = ipcRenderer.sendSync('get-extensions') as { [key: string]: IpcExtension }
    ...
    for (const key in extensions) {
      ...
      if (manifest.browser_action) {
        ...
        const icon1 = Object.keys(default_icon)[Object.keys(default_icon).length - 1]
